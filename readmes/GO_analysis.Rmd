...
#### Grouping by common GO term names 
...
```{r}
#Plotting all enriched terms in the combined genes upreg vs upreg (including those that are found in both analysis)


combined_all_ggplot <- ggplot(GO_files_combined, aes(x= New_cellType, y=term_name, colour = log10_pval, size = intersection_size)) + geom_point(alpha=0.8) +  scale_color_viridis_c(option = "plasma", name = "-log10(p)") + labs (
  title = "Terms Signficantly Enriched In Upregulated Macrocephaly/Malan/Sotos Genes", 
  subtitle = "Terms taken from Combined Genes-Upreg vs Upreg GO Analysis", 
  x= "Cell Type",
  y= "Term Name", 
  size = "Gene Count"
...
ggsave("../outputs/GO_analysis/ALL_terms_for_macro_upreg_vs_non_upreg_combined.png", combined_all_ggplot)

#Noticed different types of groupings that could be made so attempting two slightly different grouping methods 

#Group 1: Using 'Specification' 

grouped_terms <- GO_files_combined %>%
  mutate(category = case_when(
    grepl("morphogenesis", term_name, ignore.case = TRUE) ~ "morphogenesis",
    grepl("cell fate|specification", term_name, ignore.case = TRUE) ~ "specification",
    grepl("patterning", term_name, ignore.case = TRUE) ~ "patterning",
    grepl("signaling", term_name, ignore.case = TRUE) ~ "signaling",
    grepl("cancer|carcinoma|leukemia|melanoma|glioma", term_name, ignore.case = TRUE) ~ "cancer",
    grepl("infection|virus", term_name, ignore.case = TRUE) ~ "immune / infection",
    grepl("differentiation", term_name, ignore.case = TRUE) ~ "differentiation",
    grepl("development", term_name, ignore.case = TRUE) ~ "development",
    TRUE ~ "Other"
  ))
...
ggsave("../outputs/GO_analysis/ALL_terms_for_macro_upreg_vs_non_upreg_combined_grouped.png", grouped_terms_ggplot)



#Group 2: Using 'Glial' and 'Neuronal'
grouped_terms <- GO_files_combined %>%
  mutate(category = case_when(
    grepl("morphogenesis", term_name, ignore.case = TRUE) ~ "morphogenesis",
    grepl("patterning", term_name, ignore.case = TRUE) ~ "patterning",
    ...
    grepl("differentiation", term_name, ignore.case = TRUE) ~ "differentiation",
    grepl("development", term_name, ignore.case = TRUE) ~ "development",
    grepl("glial|glioma", term_name, ignore.case = TRUE) ~"glial",
    grepl("neuron|neuronal", term_name, ignore.case = TRUE) ~ "neuronal",
    TRUE ~ "Other"
  ))
...
ggsave("../outputs/GO_analysis/ALL_terms_for_macro_upreg_vs_non_upreg_combined_grouped_2.png", grouped_terms_ggplot)


```


#### 3. Visualisation of ALL TERMS enriched in JUST Upregulated Genes - Non-Upreg vs Upreg 
```{r}
install.packatges("ggtext")
library(ggtext)
